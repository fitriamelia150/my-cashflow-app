...
export default function InputMasterTransaction (){
    const [transaction, setTransaction] = useState('')

    const handleAdd = async (par: string, e: any) => {
        e.preventDefault()

        await fetch('/api/master/transaction', {
            method: 'POST',
            headers: {
        ...
        <div>
            <Card className="relative w-60 p-5 bg-slate-100 rounded-md flex items-center justify-center flex-col">
                <div className="flex justify-between items-center w-full">
                    <Input type="text" className="p-2 rounded-md w-44 bg-white w-full mb-5 text-slate-500" name="Transaction" onChange={(e) => setTransaction(e.target.value)}/>
